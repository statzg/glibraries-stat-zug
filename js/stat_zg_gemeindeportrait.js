...
require.config({
	...
	paths: {
		...
		"d3queue":"https://d3js.org/d3-queue.v3"
    },
    shim:{
		'crossfilter':{
			exports:'crossfilter'
	...
	return {
		...
		//		var uri = new URI(window.location.href);
			...
			q.await(function(error, data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13 ) {
				...
				function neuzeichnen() {
					...
					d3.json(pfadKarte, function (statesJson) {
						...
						$("#altersstruktur-chart g.stack:nth-child(3)").hide();
			...
			if (version=="online" | version=="iframe") {
				...
				generals.addDownloadButton(3);
				generals.addDownloadButtonPng(3);
				generals.addDataTablesButton(3, columns);

				var columns=["Gemeinde", "Alterskategorie", "Typ", "Anzahl"];
				generals.addDownloadButton(4);
				generals.addDownloadButtonPng(4);
				generals.addDataTablesButton(4, columns);
				
				var columns=["Gemeinde", "Sektor", "Betriebe", "Beschäftigte"];
				generals.addDownloadButton(5);
				generals.addDownloadButtonPng(5);
				generals.addDataTablesButton(5, columns);

				var columns=["Gemeinde", "Zone", "Fläche"];
				generals.addDownloadButton(6);
				generals.addDownloadButtonPng(6);
				generals.addDataTablesButton(6, columns);
				
				var columns=["Gemeinde", "Geschlecht", "Anzahl"];
				generals.addDownloadButton(7);
				generals.addDownloadButtonPng(7);
				generals.addDataTablesButton(7, columns);
				
				var columns=["Gemeinde", "Religion", "Anzahl"];
				generals.addDownloadButton(8);
