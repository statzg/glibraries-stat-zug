...
function loadStackedLine(number, csv_path, dimension, group, stack, characteristics, characteristicsStack, scale, showTotal, showAnteil, showArea, asDate, dateUnit) {
...
for (var i = 1; i < characteristicsStack.length; i++) {
	//Damit 0er nicht angezeigt werden
	Charts[number].stack(SecondaryGroups[number][characteristicsStack[i]], characteristicsStack[i], function(kv) {if (kv.value==0){return null;} else{ return kv.value; }});
	...
	d3.selectAll("circle.dot")
		...
		.on('mouseover', function(d, i) {
			...
			if (showTotal==true & showAnteil==true) {
				tiptext= "<span>"+characteristicsStack[$(this).parent().index()]+"</span><br/><span>" + label + "</span><br/><span>Anteil: " +(Math.round((d.data.value/MainGroups[number].all()[i-(($(this).parent().index())*characteristicsLength)].value)*1000)/10).toFixed(1) + '%' + "</span><br/><span>"+group+": " + wert +  "</span>";
			}
				...
				tiptext= "<span>"+characteristicsStack[$(this).parent().index()]+"</span><br/><span>" + label + "</span><br/><span>Anteil: " +(Math.round((d.data.value/MainGroups[number].all()[i-(($(this).parent().index())*characteristicsLength)].value)*1000)/10).toFixed(1) + '%' + "</span>";
			}
			else {
