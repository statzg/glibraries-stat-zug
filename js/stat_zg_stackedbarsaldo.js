...
function loadStackedBarSaldo(args) {
...
//Charts[number].xAxis().tickValues([2000, 2005, 2010, 2015]);;
...
function rotateX(){
	...
	//d3.select("#"+Atts[number].chartcontainer+" g.dc-legend-item:nth-child(1)").remove();
	//legendtranslate=d3.select("#"+Atts[number].chartcontainer+" g.dc-legend").attr("transform").replace('translate(', '').replace(')','').split(',');
	//d3.select("#"+Atts[number].chartcontainer+" g.dc-legend").attr("transform", "translate(-50,"+legendtranslate[1]+")");
	//legendsize=d3.select("#"+Atts[number].chartcontainer+" g.dc-legend").node().getBBox();
	//d3.select("#"+Atts[number].chartcontainer+" svg > g.dc-legend").append("g").attr('id', 'saldo').attr('class', 'dc-legend-item').attr("transform", "translate("+(legendsize.x+legendsize.width+15)+",0)");
	//d3.select("#"+Atts[number].chartcontainer+" #saldo").append("line").attr("x1",0).attr("y1",7).attr("x2",13).attr("y2",7).style("stroke", colorscheme[scale][1]).style("stroke-width", 2);
	//d3.select("#"+Atts[number].chartcontainer+" #saldo").append("text").text(saldolabel).attr('x',17).attr('y',11);
