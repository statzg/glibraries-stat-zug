...
define(['stat_zg_generals','dc','libs/d3-tip','libs/sankey'], function(generals,dc,d3tip,d3sankey){
	return {
		load: function(args) {
			...
			colors=['#afc369','#007ac4','#68a484','#ffdd5e','#ff8337','#ffb245','#ff403a']

				graph.nodes.forEach(function (d, i) {
					if (d.targetLinks.length==0) {
						totalImmigration=totalImmigration+graph.nodes[i].value;
						totalSaldo=totalSaldo+graph.nodes[i].value;
						console.log(totalSaldo);
					}
					if (d.sourceLinks.length==0) {
						totalEmigration=totalEmigration+graph.nodes[i].value;
						totalSaldo=totalSaldo-graph.nodes[i].value
						console.log(totalSaldo);
					}
				});
			  ...
			  var link = svg.append("g").selectAll(".sankey-link")
				...
				.enter().append("path")
					...
					.style("stroke", function(d) { 
						var bordercolor=colors[d.source.nodeid]
					...
					const startColor = colors[d.source.nodeid];
					const stopColor = colors[d.target.nodeid];
			...
			// add the rectangles for the nodes
			  node.append("rect")
				  .attr("height", function(d) { if(d.name=="Kanton Zug") {return (d.dy/d.value*totalSaldo);} else {return (Math.max(2,d.dy));} })
				  .attr("width", sankey.nodeWidth()-4)
				  .attr("y", function(d) { if(d.name=="Kanton Zug") {return (d.dy / 2 - (d.dy/d.value*totalSaldo/2));} else {return 0;} })
				  .style("fill", function(d) { 
					  return d.color = colors[d.nodeid]; })
			...
			function callTip(number){
				...
				d3.selectAll("#"+Atts[number].chartcontainer+" g.sankey-node > rect")
					...
					.on('mouseover', function(d, i) {
						...
						if (typeof d.sourceLinks !== 'undefined' && d.sourceLinks.length == 0) {
							tiptext= "<span>" + d.name + "</span><br/><span>Anteil: " + germanFormatters.numberFormat(",.1%")(d.value/totalImmigration) + "</span><br/><span>Anzahl Personen: " + germanFormatters.numberFormat(",")(d.value) + "</span>";
						} else if (typeof d.targetLinks !== 'undefined' && d.targetLinks.length == 0) {
							tiptext= "<span>" + d.name + "</span><br/><span>Anteil: " + germanFormatters.numberFormat(",.1%")(d.value/totalEmigration) + "</span><br/><span>Anzahl Personen: " + germanFormatters.numberFormat(",")(d.value) + "</span>";
						}	else {
							tiptext= "<span>" + d.name + "</span><br/><span>Bevölkerungssaldo: " + totalSaldo +"</span>";
						}
						$("#d3-tip"+number).html(tiptext)
						$("#d3-tip"+number).css("border-left", colors[d.nodeid] +" solid 5px");
				...
				d3.selectAll("#"+Atts[number].chartcontainer+" path.sankey-link")
					...
					.on('mouseover', function(d, i) {
						...
						if (d.target.name=="Kanton Zug") {
							tiptext= "<span>" + d.source.name + " -> " + d.target.name + "</span><br/><span>Anteil: " + germanFormatters.numberFormat(",.1%")(d.value/totalImmigration) + "</span><br/><span>Anzahl Personen: " +germanFormatters.numberFormat(",")(d.value) + "</span>";
						} else {
							tiptext= "<span>" + d.source.name + " -> " + d.target.name + "</span><br/><span>Anteil: " + germanFormatters.numberFormat(",.1%")(d.value/totalEmigration) + "</span><br/><span>Anzahl Personen: " +germanFormatters.numberFormat(",")(d.value) + "</span>";
						}
						$("#d3-tip"+number).html(tiptext)
						var bordercolor=mergeColors(colors[d.source.nodeid], colors[d.target.nodeid])
			...
			var columns=["Quelle", "Ziel", "Anzahl Personen"]									 
