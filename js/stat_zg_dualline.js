...
Datasets[number] = crossfilter(data),
    ...
    MainGroups[number] = MainDimensions[number].group().reduce(
        function(p,v) {
          p.push(v[group]);
          return p;
        },
...
Charts[number]
	...
	.compose([
		dc.lineChart(Charts[number])
			.group(SecondaryGroups[number][characteristicsStack[0]], characteristicsStack[0])
		...
		dc.lineChart(Charts[number])
			.group(SecondaryGroups[number][characteristicsStack[1]], characteristicsStack[1])
			.yAxisPadding("5%")
...
for (var i = 1; i < characteristicsStack.length; i++) {
	Charts[number].stack(SecondaryGroups[number][characteristicsStack[i]], characteristicsStack[i]);
}
...
function adaptYR(){
	maxwidth=0
	Charts[number].selectAll("#"+chartcontainer+" g.axis.yr > g > text")
		.attr('transform', function (d) {
			var coordx = this.getBBox().width;
			if (maxwidth < coordx) {
				maxwidth = coordx;
...
function callTip(){		
	d3.selectAll("circle.dot")
		...
		.on('mouseover', function(d, i) {
			...
			//console.log(characteristicsStack[Math.floor(i/MainGroups[number].all().length)])
			...
			if (showTotal==true & showAnteil==true) {
				tiptext= "<span>"+characteristicsStack[Math.floor(i/MainGroups[number].all().length)]+"</span><br/><span>" + label + "</span><br/><span>Anteil: " +Math.round((d.data.value[characteristics[Math.floor(i/MainGroups[number].all().length)]]/d.data.value["total"])*100).toFixed(2) + '%' + "</span><br/><span>"+group+": " + wert +  "</span>";
			}
			else if (showTotal==true) {
				tiptext= "<span>" + label + "</span><br/><span>"+characteristicsStack[Math.floor(i/MainGroups[number].all().length)]+"</span><span>: " + wert +  "</span>";
			}
			else if (showAnteil==true) {
				tiptext= "<span>"+characteristicsStack[Math.floor(i/MainGroups[number].all().length)]+"</span><br/><span>" + label + "</span><br/><span>Anteil: " +Math.round((d.data.value[characteristics[Math.floor(i/MainGroups[number].all().length)]]/d.data.value["total"])*100).toFixed(2) + '%' + "</span>";
			}
			else {
				tiptext= "<span>"+characteristicsStack[Math.floor(i/MainGroups[number].all().length)]+"</span><br/><span>" + label + "</span>";
			};
