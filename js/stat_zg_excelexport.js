...
function createXLSX () {
	...
	csv_paths.forEach(function(csv_path) {
		q.defer(d3.csv, csv_path);
	});
	...
	q.awaitAll(function(error, datas) {
		...
		datas.forEach(function(data, sheetnr) {
	
			//Get columns in datasheet
			Datasheets[number].columns = d3.keys(data[0]);
			console.log(Datasheets[number].columns)

			//Remove meta data columns from columns array
			Datasheets[number].columns.splice(-3,3);
			
			//Get D3 data object only with metadata
			Datasheets[number].meta = data.filter(function(el) {
				return el["Meta"] == 1
			});

			//Read datatypes into array
			var datatypes = Datasheets[number].meta.filter(function( el ) { return el.Type == "datatypes";});
			if (datatypes.length == 1) {
				Datasheets[number].types=datatypes[0].Content.split(',');
			}
			
			//Datasheets[number].types[1]="floatha";
			
			//Read in data as numeric or date depending on datatypes (date not yet implemented)
			data.forEach(function(d) {
				for (i=0; i<Datasheets[number].types.length; i++) {
					switch(Datasheets[number].types[i]) {
						case "integer":
						case "float":
						case "float2":
						case "float6":
						...
						case "chf":
						case "floatha":
							d[Datasheets[number].columns[i]] = +d[Datasheets[number].columns[i]];
							break;
						case "datemonthyear":
						case "dateyear":
							break;
			...
			//Get D3 data object only with data
			Datasheets[number].data = data.filter(function(el) {
				return el["Meta"] == "NA" | el["Meta"] == undefined | el["Meta"] == ""
			});
			
			//create worksheet object
			var worksheet = workbook.addWorksheet(sheetnames[sheetnr]);
			...
			for (i=0; i<Datasheets[number].columns.length; ++i) {
				Datasheets[number].cellWidths.push((d3.max(data, function(d) { return getTextWidth(d[Datasheets[number].columns[i]], "10pt arial"); })/7.025)+2);
			}
			...
			for (i=Datasheets[number].columns.length; i<100; i++) {
			...
			var defineColumns = []
			for (i=0; i<Datasheets[number].columns.length; ++i) {
				defineColumns.push({header: createRichText(Datasheets[number].columns[i], 10, false), key: Datasheets[number].columns[i], width: Datasheets[number].cellWidths[i]})
			}
			...
			var title = Datasheets[number].meta.filter(function( el ) { return el.Type == "title";});
			...
			var subtitle = Datasheets[number].meta.filter(function( el ) { return el.Type == "subtitle";});
			...
			Datasheets[number].data.forEach(function(d) {
				var cellContent=[]
				for (i=0; i<Datasheets[number].columns.length; ++i) {
					switch(Datasheets[number].types[i]) {
						case "string":
						case "year":
							var cellCT=createRichText(d[Datasheets[number].columns[i]], 10, false)
							break;
						default:
							var cellCT=d[Datasheets[number].columns[i]]
							break;
					}
			...
			var description = Datasheets[number].meta.filter(function( el ) { return el.Type == "description";});
			...
			var source = Datasheets[number].meta.filter(function( el ) { return el.Type == "source";});
			...
			for (i=headerRows; i<(Datasheets[number].data.length+headerRows+1); ++i) {
			...
			if (title.length == 1 & title[0].Content!="") {
				...
				//Merge cells
				var merge=0
				while (Datasheets[number].cellWidthsSums[merge] < ((getTextWidth(title[0].Content, "20pt arial")/7.025)+10)
						& Datasheets[number].cellWidthsSums[merge] < 60) {
					++merge
				}
				worksheet.mergeCells(1,1,1,merge)

				firstRow.height = Math.ceil(((getTextWidth(title[0].Content, "20pt arial")/7.025)+10)/Datasheets[number].cellWidthsSums[merge])*30;
			...
			for (i=headerRows+1; i<(Datasheets[number].data.length+headerRows+2); ++i) {
				Datasheets[number].cellHeights.push(12.75)
			}
			...
			if(Datasheets[number].cellWidthsSums[Datasheets[number].columns.length]<=95) {
			...
			to: {
				row: headerRows,
				column: Datasheets[number].columns.length
			}
			};
			}
			...
			//Testing rich text
			//worksheet.getCell('A10').value = createRichText("test ", 20, true)
			//worksheet.getCell('A9').value = "test "
